...
export class NavimowApiClient {
  ...
  async getMqttConnectionInfo(): Promise<NavimowMqttConnectionInfo | null> {
    ...
    this.log?.debug(
      `[Navimow API] getMqttConnectionInfo raw fields: subTopics=${debugFieldValue(subTopics)}\n` +
      `subTopics.mapChange=${debugFieldValue(sub1)} subTopics.realtime=${debugFieldValue(sub2)}\n` +
      `pwdInfo=${debugSensitiveFieldValue(pwdInfoRaw)} ak=${debugSensitiveFieldValue(akRaw)}`,
    );
