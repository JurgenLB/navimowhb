...
export class NavimowApiClient {
  ...
  async getMqttConnectionInfo(): Promise<NavimowMqttConnectionInfo | null> {
    ...
    this.log?.debug(
      `[Navimow API] getMqttConnectionInfo raw fields: ` +
      `subTopics=${debugFieldValue(subTopics)} ` +
      `subTopics.mapChange=${debugFieldValue(sub1)} ` +
      `subTopics.realtime=${debugFieldValue(sub2)} ` +
      `pwdInfo=${debugSensitiveFieldValue(pwdInfoRaw)} ` +
      `ak=${debugSensitiveFieldValue(akRaw)}`,
    );
